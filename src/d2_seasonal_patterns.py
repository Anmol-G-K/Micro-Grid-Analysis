...
def main():
    ...
    df = df.with_columns([
        # Group wind speed with wind power output (Wind Power Profile)
        (pl.col("wind_speed") * pl.col("wind_power_output")).alias("wind_power_profile"),

        # Group solar irradiance with solar power output (Solar Power Profile)
        (pl.col("solar_irradiance") * pl.col("solar_pv_output")).alias("solar_power_profile"),
    ])
    ...
    for col in ["solar_pv_output", "wind_power_output"]:
        if col not in df_pd.columns:
            continue

        # Filter out NaN values for the pivot
        df_pd_filtered = df_pd.dropna(subset=[col, "hour", "date"])

        # Create the pivot table
        pivot = df_pd_filtered.pivot_table(values=col, index="date", columns="hour", aggfunc="mean")

        # Check if pivot is empty
        if pivot.empty:
            print(f"Skipping heatmap for {col} as pivot table is empty.")
            continue

        plt.figure(figsize=(18, 8))  # make it wider
        sns.heatmap(pivot, cmap="viridis" if col == "solar_pv_output" else "inferno")
